...
# Keep child programs beside the other integration fixtures.
TEST_NATIVE_PROCESS_ROOT_PROGRAM = (
    Path(__file__).parent / "native_process_root_program.py"
)
TEST_NATIVE_PROCESS_ROOT_LAUNCHER = (
    Path(__file__).parent / "native_process_root_launcher.py"
)
...
def compile_native_sleeper(compiler: list[str], output: Path, symbol: str) -> None:
    ...
    source.write_text(
        textwrap.dedent(
            ...
            {{
                int fd = open(ready_path, O_WRONLY);
                if (fd >= 0) {{
                    write(fd, "{READY_MESSAGE.decode()}", {len(READY_MESSAGE)});
                    close(fd);
                }}
                sleep(1000);
            }}
...
def wait_for_target_ready(process: subprocess.Popen, ready_fd: int) -> None:
    """Read the FIFO signal while still detecting early namespace failures."""
    deadline = time.monotonic() + TIMEOUT
    while time.monotonic() < deadline:
        try:
            if os.read(ready_fd, len(READY_MESSAGE)) == READY_MESSAGE:
                return
...
def spawn_namespaced_target(
    ...
    ready_fifo = tmp_path / "ready_fifo"
        ...
        with subprocess.Popen(
            [
                *unshare_command,
                sys.executable,
                "-S",
                str(TEST_NATIVE_PROCESS_ROOT_LAUNCHER),
                str(target_dir),
                str(mapped_dir),
                str(ready_fifo),
            ...
            wait_for_target_ready(process, ready_fd)
            try:
                yield process
            finally:
                process.terminate()
